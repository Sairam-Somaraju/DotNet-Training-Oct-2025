...
namespace FinanceManagementSystem_Prj.Controllers
{
    public class AccountController : Controller
    {
        ...
        {
            if (!ModelState.IsValid) //If validation fails → user stays on same page
            {
                return View(model);
            }
            ...
            using (var client = new HttpClient())
            {
                ...
                var json = JsonConvert.SerializeObject(model); //“We use JsonConvert.SerializeObject to convert a  object into JSON

               //  json → the JSON string we created above
              //Encoding.UTF8 → text encoding(standard for APIs)
              // "application/json" → tells the server
        ...
        public ActionResult Login(LoginViewModel model)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                //“We use JsonConvert.SerializeObject to convert a  object into JSON
